...
var is_home = false;
var mio = false; //mobile menu is open
function home() {
...
async function get_comment_likes(is_new) {
  if (is_new == false) {
    if (localStorage.getItem("inonet_comment_likes")) {
      ...
      localStorage.setItem(
        "inonet_comment_likes",
        res.comment_likes.comment_likes
      );
...
async function like_comment(id) {
  ...
  if (res.status === "success") {
    console.log("liked");
    document
      .getElementById(`${id}-like-button`)
      .setAttribute("href", `javascript:dislike_comment('${id}');`);
...
async function dislike_comment(id) {
  ...
  if (res.status === "success") {
    ...
    document
      .getElementById(`${id}-like-button`)
      .setAttribute("href", `javascript:like_comment('${id}');`);
    document.getElementById(`${id}-like-button`).innerHTML = "پسندیدن";
    document.getElementById(`${id}-comment-likes-icon`).innerHTML =
      "favorite_border";
    document.getElementById(`${id}-comment-likes-count`).innerHTML = res.likes;
  } else if (res.status === "failed") {
    console.log(res.errors);
...
function copyaddress(id) {
  ...
  el.value = `https://narengi.net/posts/${id}/`;
  ...
  document.getElementById(
    "toast-body"
  ).innerHTML = `<p dir="ltr">https://narengi.net/posts/${id}/</p>`;
...
async function shareaddress(id) {
  try {
    await window.navigator.share({
      url: `https://narengi.net/posts/${id}/`,
    });
    console.log("Success");
  } catch (err) {
...
async function fetch_notifications() {
  ...
  if (new_notifications.length > 0) {
    ...
    document.getElementById("mobile-badge").innerHTML =
...
document.getElementById("mobile-notification-icon").addEventListener("click", () => {
    if (b) {
      clearTimeout(b);
    }
    // fetch_notifications()
...
const fetchPosts = async (url, div_id, resize) => {
  ...
  if (currentPage > 1) {
    document
      .getElementById(`loading-post-${currentPage}`)
...
async function search_query(q) {
  ...
  if (res.result.length == 0) {
    search_dropdown.innerHTML = "نتیجه‌ای یافت نشد";
  }
  Array.prototype.forEach.call(result, (r) => {
    const div = `
        <div class="search-result">
        <p><img src="${r.avatar} " alt="user avatar" width="35px" height="35px" class="user-avatar"><a href="/users/${r.username}/"> ${r.first_name} ${r.last_name} (${r.username})</a></p>
        </div>
        `;
...
search.addEventListener("keyup", (e) => {
  if (search_input.value.length > 2) {
    ...
    search_dropdown.innerHTML = "حداقل سه حرف وارد کنید";
  }
});
...
function close_mobile_menu() {
  ...
  Array.prototype.forEach.call(mobile_menu, (menu) => {
...
function mobile_new() {
  close_mobile_menu();
  document.getElementById("new-mobile-menu").classList.remove("d-none");
  mio = true;
}
function mobile_notification(load) {
  if (load) {
    close_mobile_menu();
    document
      .getElementById("notification-mobile-menu")
      .classList.remove("d-none");
    document.getElementById("notification-icon").click();
  }
  const n = document.getElementById("notification").cloneNode(true);
  document.getElementById("mobile-notification").innerHTML = "";
  document.getElementById("mobile-notification").appendChild(n);
  mio = true;
}
function mobile_settings() {
  close_mobile_menu();
  document.getElementById("settings-mobile-menu").classList.remove("d-none");
  mio = true;
}
